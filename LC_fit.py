...
def minimizer(mini_func, MiniMethod='differential_evolution', set_inc=None):
    ...
    params.add('offset', value=12.524, min=12.2,
...
def plot_result(result, ci=None):
    ...
    if ci:
        ...
        LC_sim_2sigN = LC_func(phi_arr, offset, ci['inc'][4][1]*np.pi/180,
                               dotM, dotM2, asini, fWR2O)
        ax.plot(phi_arr, LC_sim_1sigP, color='red',
                linestyle=':', label='1-$\sigma$ conf. level')
        ax.plot(phi_arr, LC_sim_1sigN, color='red', linestyle=':')
        ax.plot(phi_arr, LC_sim_2sigP, color='red',
                linestyle='-.', label='2-$\sigma$ conf. level')
        ax.plot(phi_arr, LC_sim_2sigN, color='red', linestyle='-.')
        ax.fill_between(phi_arr, LC_sim_1sigP, LC_sim_1sigN,
                        color='red', alpha=0.4)
        ax.fill_between(phi_arr, LC_sim_2sigP, LC_sim_2sigN,
                        color='red', alpha=0.2)
    fig, ax = plt.subplots(nrows=1, figsize=(7, 6))
    ax.set_title('Fit on Light-curve')
    ax.errorbar(LC_phis, LC_mags, yerr=LC_errs,
