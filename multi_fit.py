...
RVs_path = os.path.join('CCF_results', 'BAT99-32_rel',
                        line_name, 'RVs_CCF.txt')
...
def minimizer(mini_func, MiniMethod='differential_evolution', set_inc=None):
    ...
    params.add('P', value=Pini, min=Pini - 0.1 *
               Pini, max=Pini + 0.1*Pini, vary=False)
    params.add('T0', value=T0ini, min=T0ini-2*Pini,
               max=T0ini+0.1*Pini, vary=False)  # in HJD
    ...
    params.add('offset', value=12.524, min=12.4,
               max=12.8, vary=True)  # Delta mag in (13)
...
def plot_result(result, ci, plot_phase=True, interv_flag=True, plot_ellipse=True):
    ...
    if interv_flag:
        ...
        LC_sim_1sigP = LC_func(
            phi_arr, offset, ci['inc'][1][1]*np.pi/180, dotM, dotM2, asini, fWR2O)
        LC_sim_1sigN = LC_func(
            phi_arr, offset, ci['inc'][3][1]*np.pi/180, dotM, dotM2, asini, fWR2O)
        LC_sim_2sigP = LC_func(
            phi_arr, offset, ci['inc'][0][1]*np.pi/180, dotM, dotM2, asini, fWR2O)
        LC_sim_2sigN = LC_func(
            phi_arr, offset, ci['inc'][4][1]*np.pi/180, dotM, dotM2, asini, fWR2O)
    ...
    if plot_phase:
        ...
        fig, ax = plt.subplots(nrows=3, figsize=(7, 8),  gridspec_kw={'height_ratios': [1, 1, 0.7]},
                               sharex=True)
        ax[0].set_title(
            'Simultaneous fit on LC, polarimetry and radial velocities of NV-4604')
        ax[0].errorbar(LC_phis, LC_mags, yerr=LC_errs,
                       fmt='.', color='Green', alpha=0.4)
        ax[0].plot(phi_arr, LC_sim, color='red', label='Fit')
        if interv_flag:
            # For 1- and 2-sigma region
            ax[0].plot(phi_arr, LC_sim_1sigP, color='red',
                       linestyle=':', label='1-$\sigma$ conf. level')
            ax[0].plot(phi_arr, LC_sim_1sigN, color='red', linestyle=':')
            ax[0].plot(phi_arr, LC_sim_2sigP, color='red',
                       linestyle='-.', label='3-$\sigma$ conf. level')
            ax[0].plot(phi_arr, LC_sim_2sigN, color='red', linestyle='-.')
            ax[0].fill_between(phi_arr, LC_sim_1sigP,
                               LC_sim_1sigN, color='red', alpha=0.4)
            ax[0].fill_between(phi_arr, LC_sim_2sigP,
                               LC_sim_2sigN, color='red', alpha=0.2)
        ...
        if interv_flag:
            ...
            ax[1].fill_between(phi_arr, U_fit_1sigP,
                               U_fit_1sigN, color='red', alpha=0.4)
            ax[1].fill_between(phi_arr, U_fit_2sigP,
                               U_fit_2sigN, color='red', alpha=0.2)
            ax[1].fill_between(phi_arr, Q_fit_1sigP,
                               Q_fit_1sigN, color='coral', alpha=0.4)
            ax[1].fill_between(phi_arr, Q_fit_2sigP,
                               Q_fit_2sigN, color='coral', alpha=0.2)
