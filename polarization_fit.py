...
def minimizer(mini_func, MiniMethod='differential_evolution', set_inc=None):
    ...
    params.add('P', value=Pini, min=Pini-0.7 *
               Pini, max=Pini+0.7*Pini, vary=False)
    params.add('T0', value=T0ini, min=T0ini-0.5*Pini,
               max=T0ini+0.5*Pini, vary=False)  # in HJD
    if set_inc is None:
...
def plot_result(result, plot_phase=True, plot_ellipse=True):
    ...
    if plot_phase:
        plt.errorbar(pol_phis, pol_Us, yerr=pol_sig, fmt='.',
                     color='darkgreen', alpha=0.4, label='U')
        plt.errorbar(pol_phis, pol_Qs, yerr=pol_sig, fmt='.',
                     color='forestgreen', alpha=0.4, label='Q')
